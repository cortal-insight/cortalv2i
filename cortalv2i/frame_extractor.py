...
def extract_frames_from_stream(cap, output_dir, method, user_inputs, pbar=None):
    if method == '1':  # Frames per second
        fps = user_inputs['fps']
        original_fps = cap.get(cv2.CAP_PROP_FPS)
        frame_interval = int(original_fps / fps)
    elif method == '2':  # Time interval
        time_interval = user_inputs['time_interval']
    ...
    while cap.isOpened():
        ...
        if method in ['1', '2']:
            if frame_number % frame_interval == 0:
                save_frame(frame, output_dir, saved_frame_number)
    ...
    cap.release()
    print(f"Extracted {saved_frame_number} frames.")
